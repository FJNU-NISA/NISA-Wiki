...
- ⚠️ “学长学长为什么我这个出不来？”：请先确保你已具有了 [《提问的智慧》](https://lug.ustc.edu.cn/wiki/doc/smart-questions/)，完成了能做的事情，然后提供尽可能完整的相关信息给你要提问的人。
...
1. 首先，学会安装一个 Linux 系统并使用它，可以选择安装 Ubuntu 或者渗透常用的 Kali（这两个系统都是基于 debian 系的）
2. 远程命令执行，对于渗透而言，能拿到服务器的命令执行 shell 基本就意味着拿下了这台服务器，所以我们从结果出发，首先学习命令执行，也是一个很好熟练 Linux 的机会
3. LAMP/LNMP （Linux + Apache/Nginx + MySQL + PHP）基础，在 Linux 上配置
4. SQL 注入
5. PHP 特性、PHP 文件包含/文件上传
6. PHP 反序列化
7. 通用漏洞查缺补漏：XSS、SSRF、XXE 等
8. 从 Python Flask 开始的 SSTI 模板注入与衍生出的 Pyjail
9. 从 Nodejs 开始的原型链污染与沙盒逃逸
10. 从 ThinkPHP 框架开始的大型代码审计
11. 正式踏入 Web 最长的河 —— Java Security
